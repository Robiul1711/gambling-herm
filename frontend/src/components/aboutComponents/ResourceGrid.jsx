      ...
      <div className="selection:bg-teal-500/20 py-8 md:py-12 lg:py-16">
        <div className="section-padding-x">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 mb-8 items-center animate-pulse">
            {[1, 2, 3, 4, 5, 6].map((n) => (
              <div key={n} className="h-9 w-24 bg-slate-100" />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 animate-pulse">
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className="bg-white border border-slate-100 p-6 sm:p-8 flex flex-col justify-between min-h-[320px]"
              >
                <div>
                  <div className="h-4 bg-slate-100 w-1/4 mb-2" />
                  <div className="h-3 bg-slate-100 w-1/6 mb-4" />
                  <div className="h-6 bg-slate-100 w-5/6 mb-3" />
                  <div className="h-6 bg-slate-100 w-2/3 mb-4" />
                ...
                <div className="h-10 bg-slate-100 w-28" />
              </div>
            ))}
          </div>
        </div>
          ...
          {FILTERS.map((filter) => {
            ...
            return (
              <button
                ...
                className={`px-5 py-2 text-sm font-medium border transition-all duration-200 whitespace-nowrap
 ${
   isActive
     ...
     : "bg-white text-slate-600 border-slate-200 hover:border-slate-400 hover:bg-slate-50"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {filteredCards.map((card) => (
            <div
              key={card._id}
              className="bg-white border border-slate-200 p-6 sm:p-8 flex flex-col justify-between transition-shadow duration-300 hover:shadow-md min-h-[320px]"
            >
              <div>
                {/* Meta Header */}
              ...
              <div>
                <Link
                  ...
                  className="inline-flex items-center justify-center bg-Primary hover:bg-[#005580] text-white font-medium text-sm px-5 py-3 transition-colors duration-200 group"
                >
                  Read More
        ...
        {filteredCards.length === 0 && (
          <div className="text-center py-20 bg-white border border-dashed border-slate-300 mt-6">
            ...
            {/* Footer info text */}
        <div className="mt-10 text-slate-700 text-sm sm:text-base font-normal">
          <p>
            An archive and RSS feed are coming soon. For media enquiries,{" "}
            <a
              href="mailto:press@gamblingharm.com"
              className="text-[#156E94] underline hover:text-Primary font-medium transition-colors"
            >
