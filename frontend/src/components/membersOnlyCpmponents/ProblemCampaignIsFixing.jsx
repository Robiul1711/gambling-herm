...
const ProblemCampaignIsFixing = () => {
  ...
  const analyticalPoints = [
    {
      highlight:
        "The current land-based self-exclusion scheme (BOSE / SENSE / BACTA)",
      ...
      <div className="">
        ...
        <div className=" border-t border-dashed border-gray-200 divide-y divide-dashed divide-gray-200">
          {analyticalPoints.map((point, index) => (
            <div
              key={index}
              className="py-6 text-sm md:text-base leading-relaxed text-gray-600"
            >
              <span className="font-bold text-gray-900">
                {point.highlight}{" "}
              </span>
              <span className=" text-gray-700">{point.body}</span>
            </div>
          ))}
        </div>
      </div>
