      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-[#1f2937]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {tools.map((tool, index) => (
            <div
              key={index}
              className="bg-white border border-gray-150/80 rounded-xl p-5 lg:p-8 shadow-xs flex flex-col justify-between hover:shadow-md transition-shadow duration-300"
            >
              <div>
              ...
              <div>
                <Link
                  to={tool.to}
                  className="inline-flex items-center gap-2 bg-[#0B678F] hover:bg-[#095272] text-white font-semibold text-sm py-3 px-5 rounded-md transition-colors"
                >
                  {tool.buttonText}
                  <span>&rarr;</span>
                </Link>
