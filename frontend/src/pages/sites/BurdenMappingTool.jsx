...
const MAP_BANDS = [
  { t: "Well above the GB average", c: "#0A4561", min: 1.5 },
...
const R37 = [4.9, 4.12, 4.51, 3.39, 2.4, 1.28, 0.77];
const R12 = [11.69, 12.28, 11.7, 9.51, 6.93, 4.31, 2.83];
const RM8 = 3.74,
  RF8 = 1.63,
  RM37 = 4.12,
  RF37 = 2.22,
...
const RRE8 = 2.5,
  RRE17 = 1.2;
const NAT_AB = [10.54, 17.01, 16.9, 15.45, 16.17, 12.11, 11.82];
const NAT_MALE = 48.38,
  NAT_ETH = 18.3;
...
const RD37 = [4.34, 3.26, 3.04, 2.8, 1.8];
const RD12 = [10.02, 8.98, 8.77, 8.34, 7.79];
const ENG_Q = [19.73, 20.42, 20.3, 19.87, 19.67];
const BLEND_K = 100000;

const OHID_LO_PER_ADULT = 23,
  OHID_HI_PER_ADULT = 39;
const VIC_BASE_LO = 60,
  VIC_BASE_MID = 97,
  VIC_BASE_HI = 129;
const HSE_P8 = 0.373,
  HSE_P17 = 3.538;
const SEV_HIGH = 0.355,
  SEV_ATRISK = 0.645;
const VIC_SH = [
...
export default function BurdenMappingTool() {
  ...
  const calcResults = useMemo(() => {
    ...
    if (selectedLA) {
      ...
      const yScale = young / NAT_YOUNG,
        oScale = (100 - young) / (100 - NAT_YOUNG);
    ...
    const sexAdj = (rm, rf) =>
      (male * rm + (100 - male) * rf) / (NAT_MALE * rm + (100 - NAT_MALE) * rf);
    const ethAdj = (rr) =>
      (100 - eth + eth * rr) / (100 - NAT_ETH + NAT_ETH * rr);

    const visNum = parseFloat(vis.replace(/[^0-9.]/g, "")) || 0;
    const dens = selectedLA
      ? (selectedLA.g.reduce((a, b) => a + b, 0) /
          (expPop(selectedLA) + visNum)) *
        1e5
      : GB_DENS;
    const dRatio = Math.min(
      Math.max(dens / GB_DENS, SUPPLY_CAP[0]),
      SUPPLY_CAP[1],
    );
    const supAdj = Math.pow(dRatio, sup);

    const depAdj = (R) =>
      selectedLA && selectedLA.d
        ? selectedLA.d.reduce((a, v, i) => a + v * R[i], 0) /
          ENG_Q.reduce((a, v, i) => a + v * R[i], 0)
        : 1;

    let adj8 =
      ageAdj(R8) * sexAdj(RM8, RF8) * ethAdj(RRE8) * depAdj(RD8) * supAdj;
    let adj37 =
      ageAdj(R37) * sexAdj(RM37, RF37) * ethAdj(RRE17) * depAdj(RD37) * supAdj;
    let adj12 =
      ageAdj(R12) * sexAdj(RM12, RF12) * ethAdj(RRE17) * depAdj(RD12) * supAdj;
    ...
    if (basisMode === "ga" && selectedLA && selectedLA.ga) {
      ...
      const bl = (gaR, mA) =>
        Math.exp(w * Math.log(gaR) + (1 - w) * Math.log(mA));
    ...
    const vicAdj =
      SEV_HIGH * (p8e / HSE_P8) + SEV_ATRISK * ((p12e + p37e) / HSE_P17);
    ...
    const yldCiLo =
      (n12 * (7.9 / 8.8) * dw12 +
        ...
        n8 * (2.2 / 2.7) * dw8) *
        af +
      yldAo * (8 / 9) * ((7.9 / 8.8 + 2.8 / 3.1 + 2.2 / 2.7) / 3) +
      yldKids * (8 / 9);
    const yldCiHi =
      (n12 * (9.8 / 8.8) * dw12 +
        n37 * (3.6 / 3.1) * dw37 +
        n8 * (3.2 / 2.7) * dw8) *
        af +
      yldAo * (10 / 9) * ((9.8 / 8.8 + 3.6 / 3.1 + 3.2 / 2.7) / 3) +
      yldKids * (10 / 9);
    ...
    function yldForBands(P) {
      ...
      return (
        (((pop * a) / 100) * dw12 +
          ...
          af +
        nA *
          (0.3727 * dwao12 + 0.2725 * dwao37 + 0.3548 * dwao8) *
          af *
          AO_DEDUP +
  ...
  const driversInfo = useMemo(() => {
    const gbDens = 11.7;
    const dens = LAS.map((la) =>
      Math.max((la.g.reduce((x, y) => x + y, 0) / expPop(la)) * 1e5, 0),
    );
    const hiIdx = LAS.map((la, i) => i).filter((i) => {
      const item = mapPrevAll[i];
      return item && item.ratio >= 1.15;
    });
    const avgDensHi = hiIdx.length
      ? hiIdx.reduce((a, i) => a + dens[i], 0) / hiIdx.length
      : gbDens;
    return {
  ...
  const tornadoItems = useMemo(() => {
    ...
    const gY = (p12e, p37e, p8e, w12, w37, w8, aoPct, kidScale) => {
      ...
      const ks =
        kidScale === undefined
          ? (s.p12 + s.p37 + s.p8) / GSGB_P1PLUS
          : kidScale;
      const n12 = (s.pop * p12e) / 100,
        n37 = (s.pop * p37e) / 100,
        n8 = (s.pop * p8e) / 100;
      const nAo = (s.pop * a) / 100;
      const ya =
        (nAo * 0.3727 * w12 + nAo * 0.2725 * w37 + nAo * 0.3548 * w8) *
        AO_DEDUP;
    ...
    const items = [
      {
        l: "Which survey (APMS 2023/24 ↔ GSGB 2024)",
        lo: Math.min(...across),
        hi: Math.max(...across),
      },
    ];
    ...
    if (surveyMode === "gsgb") {
      items.push({
        l: "PGSI 8+ sampling CI (2.2–3.2%)",
        lo: gY(s.p12e, s.p37e, 2.2 * s.adj8, ...cur),
        hi: gY(s.p12e, s.p37e, 3.2 * s.adj8, ...cur),
      });
      items.push({
        l: "PGSI 1–2 sampling CI (7.9–9.8%)",
        lo: gY(7.9 * s.adj12, s.p37e, s.p8e, ...cur),
        hi: gY(9.8 * s.adj12, s.p37e, s.p8e, ...cur),
      });
      items.push({
        l: "PGSI 3–7 sampling CI (2.8–3.6%)",
        lo: gY(s.p12e, 2.8 * s.adj37, s.p8e, ...cur),
        hi: gY(s.p12e, 3.6 * s.adj37, s.p8e, ...cur),
      });
    }
    ...
    const f0 = Math.pow(s.dRatio, 0 - sup),
      f3 = Math.pow(s.dRatio, 0.3 - sup);
    items.push({
  ...
  useEffect(() => {
        ...
        fetch(bURL)
          ...
          .then((gj) => {
            ...
            for (let i = 0; i < codes.length; i += 60)
              chunks.push(codes.slice(i, i + 60));

            Promise.all(
              chunks.map((ch) => {
                const w = encodeURIComponent(
                  "LSOA11CD IN (" + ch.map((c) => `'${c}'`).join(",") + ")",
                );
                return fetch(
                  `https://services1.arcgis.com/ESMARspQHYMw9BZ9/ArcGIS/rest/services/Index_of_Multiple_Deprivation_Dec_2019_Lookup_in_England_2022/FeatureServer/0/query?where=${w}&outFields=LSOA11CD,IMD19&returnGeometry=false&f=json`,
                ).then((r) => r.json());
              }),
            ).then((res) => {
              const rank = {};
              res.forEach((r) =>
                (r.features || []).forEach((f) => {
                  rank[f.attributes.LSOA11CD] = f.attributes.IMD19;
                }),
              );
              clearLayer(imdLayerRef);
              imdLayerRef.current = L.geoJSON(gj, {
                style: (f) => {
                  ...
                  const q = Math.min(
                    4,
                    Math.floor((Math.ceil(rk / 3284.4) - 1) / 2),
                  );
                  return {
                    color: "#00000000",
                    weight: 0,
                    fillColor: IMD_COLS[q],
                    fillOpacity: q < 3 ? 0.4 : 0.1,
                  };
                },
                interactive: false,
              }).addTo(map);
          ...
          .catch(() =>
            setOvStatus("Deprivation layer unavailable (network restricted)"),
          );
      }
    } else {
      clearLayer(imdLayerRef);
    ...
    if (layerSch) {
      ...
      fetch("https://overpass-api.de/api/interpreter", {
          ...
          (d.elements || []).forEach((e) => {
            ...
            if (lat != null && lon != null) {
              L.circleMarker([lat, lon], {
                ...
                .bindTooltip((e.tags && e.tags.name) || "School")
                .addTo(grp);
            }
          });
        ...
        .catch(() =>
          setOvStatus("Schools layer unavailable (network restricted)"),
        );
    } else {
      clearLayer(schLayerRef);
    }
    ...
    if (layerHos) {
      ...
      fetch("https://overpass-api.de/api/interpreter", {
          ...
          (d.elements || []).forEach((e) => {
            ...
            if (lat != null && lon != null) {
              L.circleMarker([lat, lon], {
                ...
                .bindTooltip(
                  (e.tags && e.tags.name) || "Hospital / Mental Health",
                )
                .addTo(grp);
            }
          });
        ...
        .catch(() =>
          setOvStatus("Hospitals layer unavailable (network restricted)"),
        );
    } else {
      clearLayer(hosLayerRef);
    }
  ...
  const handleCopySummary = () => {
    const d = calcResults;
    const txt = `Gambling harm burden estimate, ${selectedLA ? selectedLA.n + ", " : ""}adult population ${fmtPop(d.pop)}
...
Deprivation is modelled for English authorities (GSGB table D.8 × IMD 2019 quintile shares; PGSI 8+ runs 5.9% in England's most deprived quintile vs 1.0% in the least); Scottish and Welsh authorities carry no structural deprivation term. Online gambling has no local supply term, areas with high online penetration are understated. Estimates are ecological (population-level), not individual-level risk. Children are counted via the OHID proxy method (see methods). Causal-attribution factor applied to all weights: ${d.af.toFixed(2)}${d.af < 1 ? " (below the published 1.0 default)" : " (published default: observed wellbeing gap fully attributed)"}. Note: 'years of health and wellbeing lost' uses the Moayeri (2020) SF-6D utility-decrement basis and is not comparable to GBD YLD. Directly elicited disability weights (Tulloch et al., 2026) measure a different quantity on a different scale and are deliberately not offered in this tool, because monetising them at a £/QALY value would price one metric on another metric's scale. Source: Gambling Harm UK Burden Calculator v2.6 (defaults frozen as v2.5, 11 July 2026); full parameter audit and methodology at the tool page.`;
  ...
  const maxDev = useMemo(() => {
    ...
    return Math.max(
      ...tornadoItems.map((it) =>
        Math.max(Math.abs(it.hi - base), Math.abs(it.lo - base)),
      ),
      1,
    );
  ...
  return (
    <div
      className={`burden-mapping-page px-4  text-[#1A1A1A] bg-[#FAFAFA] min-h-screen ${isAdvanced ? "advanced-mode" : "basic-mode"}`}
    >
      {/* Header Banner */}
      <div className="border-b-[3px] border-[#1A1A1A] bg-white py-7 ">
        <div className="max-w-[1120px] mx-auto">
          ...
          <h1 className="font-extrabold text-3xl sm:text-4xl md:text-[40px] leading-[1.08] tracking-tight max-w-[780px]">
            How much health and wellbeing is gambling taking from{" "}
            <em className="not-italic text-[#0A4561]">your population</em>?
          </h1>
          <p className="max-w-[660px] mt-3 text-[#404040] text-[15px] leading-relaxed">
            This calculator estimates the health burden of gambling across the
            full spectrum of harm, not just the most severe cases, using the
            health-utility evidence behind the GHUK Health Needs Assessment
            (Moayeri, 2020). It does not give you one number. It gives you a
            defensible number, on a survey basis you have chosen and can cite,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
            <div className="border border-[#E5E5E5]  p-3 text-[12.5px] text-[#404040] bg-white">
              <b className="block text-[#166F94] text-[11px] tracking-wider uppercase mb-1 font-bold">
                1 · Select
              </b>
              Choose your local authority; population, age, sex and ethnic
              structure load automatically from ONS and Census data.
            </div>
            <div className="border border-[#E5E5E5]  p-3 text-[12.5px] text-[#404040] bg-white">
              <b className="block text-[#166F94] text-[11px] tracking-wider uppercase mb-1 font-bold">
                2 · Choose your survey
              </b>
              Three UK surveys measure gambling harm and disagree by about five
              times at the severe end, because they asked differently. Pick the
              one you are prepared to defend. Everything else follows from it.
            </div>
            <div className="border border-[#E5E5E5]  p-3 text-[12.5px] text-[#404040] bg-white">
              <b className="block text-[#166F94] text-[11px] tracking-wider uppercase mb-1 font-bold">
                3 · Use
              </b>
              Copy the fully cited summary into your JSNA, board paper or
              licensing statement, or download a PDF. Switch to Advanced to
              adjust any assumption. Methods and audit trail are linked below.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1120px] mx-auto py-8">
        <main className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_400px] gap-7">
          ...
          <section className="space-y-5">
            ...
            <div
              className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs"
              id="pop-card"
            >
              ...
              <p className="text-[13px] text-[#404040] mb-4">
                Choose a local authority, ONS mid-2023 population and Census
                2021 ethnicity load automatically, or enter figures manually
                below.
              </p>
              ...
              <div className="relative mb-3">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(val);
                    const match = LAS.find(
                      (l) => l.n.toLowerCase() === val.trim().toLowerCase(),
                    );
                    handleSelectLA(match || null);
                  }}
                  placeholder="Start typing a local authority…"
                  className="w-full font-['IBM_Plex_Mono',monospace] text-base font-semibold p-3 border-2 border-[#1A1A1A]  bg-[#FAFAFA] text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#166F94]"
                />
              ...
              {!selectedLA && (
                <p className="text-xs text-[#404040] mb-3">
                  Selecting an authority loads its Census ethnicity profile,
                  licensed-premises counts and the local supply map (Gambling
                  Commission register).
                </p>
              )}
                ...
                <div className="mb-4 p-3 bg-[#FAFAFA] border border-[#E5E5E5] ">
                  <div className="text-xs font-semibold mb-2 text-[#1A1A1A]">
                    ...
                    <span className="font-normal text-[#404040]">
                      · England &amp; Wales average shown as ▲
                    </span>
                  </div>
                  {selectedLA.e ? (
                    <div className="space-y-1">
                      {Object.keys(ETH_LABELS).map((k) => {
                        ...
                        return (
                          <div
                            key={k}
                            className="grid grid-cols-[64px_1fr_46px] items-center gap-2 text-xs"
                          >
                            ...
                            <div className="relative h-2.5 bg-white border border-[#E5E5E5] rounded-xs">
                              <div
                                className="h-full bg-[#166F94] rounded-xs"
                                style={{ width: `${v}%` }}
                              ></div>
                              <div
                            ...
                            <b className="font-['IBM_Plex_Mono',monospace] text-right text-[#1A1A1A]">
                              {v.toFixed(1)}%
                            </b>
                          </div>
                        );
                    ...
                    <p className="text-xs text-[#404040]">
                      Ethnicity data not available for Scottish authorities
                      (Census 2021 covers England &amp; Wales); the ethnicity
                      scale keeps the national default.
                    </p>
                  )}
                  <p className="text-[11px] text-[#404040] mt-2 italic">
                    GSGB shows elevated PGSI rates among some minority ethnic
                    groups, but sample sizes preclude a robust local adjustment;
                    areas with larger minority ethnic populations should treat
              ...
              <input
                ...
                className="w-full font-['IBM_Plex_Mono',monospace] text-xl font-semibold p-3 border-2 border-[#1A1A1A]  bg-[#FAFAFA] text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#166F94]"
              ...
              {selectedLA && (
                <div className="mt-4 p-3 bg-[#FAFAFA] border border-[#E5E5E5] ">
                  <div className="text-xs font-semibold mb-2 text-[#1A1A1A]">
                    Licensed gambling premises (Gambling Commission register)
                  </div>
                  <div className="space-y-1 text-xs text-[#404040]">
                    <div className="flex justify-between py-0.5">
                      <span>Betting shops</span>
                      <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                        {selectedLA.g[0].toLocaleString("en-GB")}
                      </b>
                    </div>
                    <div className="flex justify-between py-0.5">
                      <span>Adult gaming centres</span>
                      <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                        {selectedLA.g[1].toLocaleString("en-GB")}
                      </b>
                    </div>
                    <div className="flex justify-between py-0.5">
                      <span>Bingo</span>
                      <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                        {selectedLA.g[2].toLocaleString("en-GB")}
                      </b>
                    </div>
                    <div className="flex justify-between py-0.5">
                      <span>Casinos</span>
                      <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                        {selectedLA.g[3].toLocaleString("en-GB")}
                      </b>
                    </div>
                  ...
                  <p className="text-[11px] text-[#404040] mt-2 italic">
                    Premises data is admissible evidence for your Local Area
                    Profile and statement of licensing principles.
                  </p>
                </div>
              )}
              ...
              {isAdvanced && (
                <div className="space-y-4 pt-4 border-t border-dashed border-[#E5E5E5] mt-4">
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="font-semibold text-sm">
                        Share of adults aged 18–34
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] font-semibold text-[#166F94]">
                        {Math.round(young)}%
                      </span>
                    </div>
                    <p className="text-xs text-[#404040] mb-2">
                      From ONS mid-year estimates (GB ≈ 27.6%). PGSI 8+ runs
                      5.3% at 18–24 to 0.3% at 75+.
                    </p>
                    <input
                      type="range"
                      min="0"
                      ...
                      value={young}
                      onChange={(e) => {
                        setYoung(parseFloat(e.target.value));
                        markCustomScen();
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
                    <div className="flex justify-between text-[11px] text-[#404040] font-['IBM_Plex_Mono',monospace]">
                      <span>0%</span>
                      <span>30%</span>
                      <span>60%</span>
                    </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#404040] mb-2">
                      From ONS mid-year estimates (GB ≈ 48.4%). PGSI 8+ is 3.7%
                      in men vs 1.6% in women.
                    </p>
                    <input
                      type="range"
                      min="40"
                      ...
                      value={male}
                      onChange={(e) => {
                        setMale(parseFloat(e.target.value));
                        markCustomScen();
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="font-semibold text-sm">
                        Share of adults from minority ethnic groups
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] font-semibold text-[#166F94]">
                        {eth.toFixed(1)}%
                      </span>
                    ...
                    <p className="text-xs text-[#404040] mb-2">
                      Census 2021 (auto-filled on selecting an English or Welsh
                      authority).
                    </p>
                    <input
                      type="range"
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="font-semibold text-sm">
                        Visitor footfall: daily visitor-equivalents (optional)
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] font-semibold text-[#166F94]">
                        {vis
                          ? `${Math.round(parseFloat(vis)).toLocaleString("en-GB")}/day`
                          : "–"}
                      </span>
                    </div>
                    <input
                      ...
                      className="w-full p-2 text-sm border border-[#E5E5E5]  font-['IBM_Plex_Mono',monospace]"
                    />
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="font-semibold text-sm">
                        Supply effect: elasticity of harm to premises density
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] font-semibold text-[#166F94]">
                        {sup.toFixed(2)}
                      </span>
                    ...
                    <p className="text-xs text-[#404040] mb-2">
                      Applies (local density ÷ GB density)^ε to prevalence.
                      Modest default 0.10.
                    </p>
                    <input
                      type="range"
                      ...
                      value={sup}
                      onChange={(e) => {
                        setSup(parseFloat(e.target.value));
                        markCustomScen();
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
                  ...
                  <p className="text-xs text-[#404040] pt-2 border-t border-[#E5E5E5]">
                    Combined local demographic adjustment: ×
                    {calcResults.adj8.toFixed(2)} (PGSI 8+), ×
                    {calcResults.adj37.toFixed(2)} (3–7), ×
                    {calcResults.adj12.toFixed(2)} (1–2)
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
              ...
              <p className="text-[13px] text-[#404040] mb-4 leading-relaxed">
                All 207 upper-tier authorities in four bands against the GB
                average. No ranking and no local point estimate, because the
                survey behind it cannot support either. The pattern is the
                finding, not any one area’s position.
              </p>

              <div className="grid grid-cols-1 md:grid-cols-[minmax(0,300px)_1fr] gap-5 items-start">
                {/* SVG UK Choropleth */}
                <div className="bg-[#FAFAFA] border border-[#E5E5E5]  p-2 flex items-center justify-center">
                  <svg
                    viewBox="0 0 520 1049"
                    className="w-full max-w-[280px] h-auto drop-shadow-xs"
                    role="img"
                    aria-label="Map of Great Britain shaded by estimated PGSI 1 plus prevalence per authority"
                  >
                ...
                <div className="space-y-3">
                  <div className="text-xs font-semibold text-[#1A1A1A]">
                    Estimated PGSI 1+ against the GB average
                  </div>
                  <div className="space-y-1.5">
                    {MAP_BANDS.map((b, i) => (
                      <div
                        key={b.t}
                        className="grid grid-cols-[14px_1fr_auto] gap-2 items-center text-xs text-[#404040]"
                      >
                        <i
                          className="w-3.5 h-3.5 rounded-xs border border-[#E5E5E5]"
                  ...
                  {/* Here line */}
                  <div className="mt-3 p-2.5 bg-[#FAFAFA] border border-[#E5E5E5]  text-xs leading-relaxed">
                    {selectedLA && selectedPrevItem ? (
                      <div className="flex gap-2 items-start text-[#1A1A1A]">
                        <i
                          className="w-3.5 h-3.5 rounded-xs border border-[#E5E5E5] shrink-0 mt-0.5"
                          style={{ background: selectedPrevItem.band.c }}
                        ></i>
                        <span>
                          <b>{selectedLA.n}</b> sits{" "}
                          {selectedPrevItem.band.t
                            .toLowerCase()
                            .replace("the gb average", "the GB average")}
                          .
                          {selectedLA.a < 100000 &&
                            " This authority is small enough that the estimate carries wide uncertainty; treat the band as indicative only."}
                        </span>
                      </div>
                    ) : (
                      <span className="text-[#404040] italic">
                        Select an authority above to see which band it falls in.
                      </span>
                    )}
                  </div>
                  ...
                  <p className="text-xs text-[#404040] italic mt-2 leading-relaxed">
                    Click any authority to select it. London boroughs are small
                    at this scale; use the search box above to find one.
                    Boundaries: ONS (ultra-generalised), December 2023.
                  </p>
                  ...
                  <details className="mt-3 text-xs text-[#404040]">
                    ...
                    <div className="mt-2 space-y-2 text-xs text-[#404040] leading-relaxed">
                      <p>
                        • <b className="text-[#1A1A1A]">Age.</b> PGSI 8+ runs
                        5.3% among 18–24s against 0.25% among over-75s (GSGB
                        table D.4), so younger populations carry more harm
                        before anything local is considered.
                      </p>
                      <p>
                        • <b className="text-[#1A1A1A]">Deprivation.</b> 5.9% at
                        PGSI 8+ in England’s most deprived IMD quintile against
                        1.0% in the least, and 21% of premises sit in the most
                        deprived decile against 2% in the least.
                      </p>
                      <p>
                        • <b className="text-[#1A1A1A]">Supply.</b> The{" "}
                        {driversInfo.hiCount} authorities above the GB average
                        average {driversInfo.avgDensHi} licensed premises per
                        100,000 against {driversInfo.gbDens} nationally. Supply
                        is counted, not modelled, so this comparison is firmer
                        than the prevalence bands themselves.
                      </p>
                      <p>
                        • <b className="text-[#1A1A1A]">What this is not.</b>{" "}
                        These are population characteristics, not council
                        failures. An authority appears in a higher band largely
                        because of who lives there and what has been licensed
                        near them.
                      </p>
                    </div>
                  </details>
              ...
              <p className="text-xs text-[#404040] italic mt-4 pt-3 border-t border-[#E5E5E5] leading-relaxed">
                Read this as four groups, not 207 positions. Bands are set at
                15% and 50% either side of the GB average, and an authority near
                a band edge could sit either side of it on a different survey
                draw. Authorities under roughly 100,000 people carry the widest
                uncertainty of all, and small-area estimates should not be
                presented to spurious precision (HNA v9.9). The one figure on
                this page that is counted rather than modelled is premises
                density, which is why it is reported separately and exactly.
              </p>
            </div>
            ...
            {selectedLA && (
              <div
                className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs relative z-0 isolate"
                id="localmap-card"
              >
                <h2 className="font-bold text-sm tracking-wider uppercase mb-1 text-[#1A1A1A]">
                  {isAdvanced && <span>6 · </span>}Local area profile: supply
                  map, {selectedLA.n}
                </h2>
                <p className="text-[13px] text-[#404040] mb-3">
                  Every licensed gambling premises in {selectedLA.n} (Gambling
                  Commission register, geocoded).
                </p>
                ...
                <div className="flex flex-wrap gap-4 text-xs mb-3 items-center">
                  ...
                  {ovStatus && (
                    <span className="text-[#0A4561] text-[11px] font-medium animate-pulse">
                      {ovStatus}
                    </span>
                  )}
                </div>
                ...
                <div
                  ...
                  className="border border-[#E5E5E5]  overflow-hidden bg-[#FAFAFA] relative z-0 isolate"
                ></div>
            ...
            <div className="bg-white border-2 border-[#1A1A1A] rounded-[10px] p-5 shadow-xs">
              <h2 className="font-bold text-sm tracking-wider uppercase mb-1 text-[#1A1A1A]">
                The burden in your area
              </h2>
              <div className="font-['IBM_Plex_Mono',monospace] text-xs text-[#404040] pb-2 mb-3 border-b border-[#E5E5E5]">
                Population: {fmtPop(calcResults.pop)} adults
                {selectedLA
                  ? `, ${selectedLA.n}`
                  : calcResults.pop === 53529600
                    ? ", Great Britain (all 207 authorities)"
                    : ", custom population"}
              </div>
              ...
              <div className="py-2 pb-4 border-b border-[#1A1A1A]">
                ...
                <div className="text-[13px] text-[#404040] mt-1">
                  years of health and wellbeing lost to gambling harm each year,
                  including people affected by someone else's gambling and
                  children (HNA utility-decrement basis)
                </div>
                ...
                {calcResults.pop > 0 && calcResults.pop < 100000 && (
                  <div className="mt-2 text-xs bg-[#FBF4E7] text-[#5C4210] p-2 ">
                    This population is below the ~100,000 reliability floor for
                    small-area estimates. Treat this as an order of magnitude.
                  </div>
                )}
              </div>
              ...
              <div className="py-3 border-b border-dashed border-[#E5E5E5]">
                ...
                <div className="text-[13px] text-[#404040] mt-0.5">
                  licensed gambling premises per 100,000 people{" "}
                  {selectedLA
                    ? `in ${selectedLA.n} (footfall-adjusted)`
                    : "across Great Britain"}
                  . Counted from the Gambling Commission register, not modelled.
                </div>
              </div>
              ...
              <div className="py-3 border-b border-dashed border-[#E5E5E5]">
                ...
                <div className="text-[13px] text-[#404040] mt-0.5">
                  adults at PGSI 8+ likely <em>not</em> in structured treatment
                  (estimated ≈95% treatment gap).
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-[#E5E5E5] text-[12.5px] text-[#404040] space-y-2">
                <div>
                  <strong className="font-['Hanken_Grotesk',sans-serif] uppercase tracking-wider text-xs text-[#1A1A1A]">
                    Costs for policy use.{" "}
                  </strong>
                  OHID's (2023) cost-of-illness scales to{" "}
                  <strong className="font-['IBM_Plex_Mono',monospace] text-[#1A1A1A]">
                    {fmtMoney(calcResults.ohidLo)}–
                    {fmtMoney(calcResults.ohidHi)}/yr
                  </strong>
                  ; full social-cost accounting (Browne et al., 2025) gives{" "}
                  <strong className="font-['IBM_Plex_Mono',monospace] text-[#1A1A1A]">
                    ≈ {fmtMoney(calcResults.vicMid)}/yr
                  </strong>
                  .
                </div>
                ...
                <details className="text-xs">
                  ...
                  <div className="mt-2 p-3 bg-[#FAFAFA] border border-[#E5E5E5]  space-y-1">
                    {VIC_SH.map(([label, share]) => (
                      <div key={label} className="flex justify-between py-0.5">
                        <span>{label}</span>
                        <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                          {fmtMoney(calcResults.vicMid * share)}
                        </b>
                      </div>
                    ))}
                  </div>
              ...
              <div className="pt-3 border-t border-dashed border-[#E5E5E5] mt-2">
                ...
                <div className="text-[13px] text-[#404040]">
                  societal <em>wellbeing valuation</em> (£
                  {(calcResults.qalyK * 1000).toLocaleString("en-GB")}/healthy
                  year; HM Treasury Green Book framing).
                </div>
              </div>
              ...
              <div className="mt-4 space-y-2">
                <button
                  ...
                  className={`w-full py-3 px-4 font-bold text-xs uppercase tracking-wider  text-white transition-colors cursor-pointer ${copySuccess ? "bg-[#1A1A1A]" : "bg-[#166F94] hover:bg-[#0A4561]"}`}
                >
                ...
                <button
                  ...
                  className="w-full py-3 px-4 font-bold text-xs uppercase tracking-wider  text-white bg-[#1A1A1A] hover:bg-[#404040] transition-colors cursor-pointer"
                >
            ...
            {/* CARD: Who it affects */}
            <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
              <h2 className="font-bold text-sm tracking-wider uppercase mb-3 text-[#1A1A1A]">
                Who it affects
              </h2>
              ...
              <div className="py-2 border-b border-[#E5E5E5]">
                ...
                <div className="text-[13px] text-[#404040]">
                  adults experiencing gambling harm (PGSI 1+)
                </div>

                <div className="mt-2 p-3 bg-[#FAFAFA] border border-[#E5E5E5]  text-xs space-y-1">
                  <div className="font-semibold text-[#1A1A1A]">
                    Estimated profile of adults at PGSI 8+
                  </div>
                  <div className="flex justify-between">
                    <span>Aged 18–34</span>
                    <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                      {Math.round(calcResults.profYoung)}% (vs{" "}
                      {Math.round(calcResults.bands[0] + calcResults.bands[1])}
                      %)
                    </b>
                  </div>
                  <div className="flex justify-between">
                    <span>Men</span>
                    <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                      {Math.round(calcResults.profMale)}% (vs {Math.round(male)}
                      %)
                    </b>
                  </div>
                  <div className="flex justify-between">
                    <span>Minority ethnic groups</span>
                    <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                      {Math.round(calcResults.profEth)}% (vs {Math.round(eth)}%)
                    </b>
                  </div>
                </div>
              </div>
              ...
              <div className="py-3 border-b border-[#E5E5E5]">
                ...
                <div className="text-[13px] text-[#404040]">
                  adults affected by someone else's gambling
                </div>

                <div className="mt-2 p-3 bg-[#FAFAFA] border border-[#E5E5E5]  text-xs space-y-1">
                  <div className="font-semibold text-[#1A1A1A]">
                    Whose gambling harms them
                  </div>
                  <div className="flex justify-between">
                    <span>Someone at PGSI 8+</span>
                    <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                      {round3sf(calcResults.nAo8).toLocaleString("en-GB")}{" "}
                      people ·{" "}
                      {round3sf(calcResults.yldAo8).toLocaleString("en-GB")} yrs
                    </b>
                  </div>
                  <div className="flex justify-between">
                    <span>Someone at PGSI 3–7</span>
                    <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                      {round3sf(calcResults.nAo37).toLocaleString("en-GB")}{" "}
                      people ·{" "}
                      {round3sf(
                        calcResults.yldAo * (calcResults.shPct[1] / 100),
                      ).toLocaleString("en-GB")}{" "}
                      yrs
                    </b>
                  </div>
                  <div className="flex justify-between">
                    <span>Someone at PGSI 1–2</span>
                    <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                      {round3sf(calcResults.nAo12).toLocaleString("en-GB")}{" "}
                      people
                    </b>
                  </div>
                  <div className="pt-1 text-[11px] text-[#404040] italic">
                    {calcResults.aoShare8}% of the health burden on family and
                    friends traces to severe gambling (PGSI 8+).
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3">
                <div className="text-[13px] font-semibold mb-2 text-[#1A1A1A]">
                  Where the health burden sits
                </div>
                <div className="flex h-5.5  overflow-hidden border border-[#E5E5E5]">
                  <div
                    style={{
                      width: `${calcResults.shLow * 100}%`,
                      background: "#CFF1FF",
                    }}
                  ></div>
                  <div
                    style={{
                      width: `${calcResults.shMod * 100}%`,
                      background: "#2989B0",
                    }}
                  ></div>
                  <div
                    style={{
                      width: `${calcResults.shHigh * 100}%`,
                      background: "#0A4561",
                    }}
                  ></div>
                  <div
                    style={{
                      width: `${calcResults.shAo * 100}%`,
                      background: "#7E9296",
                    }}
                  ></div>
                  <div
                    style={{
                      width: `${calcResults.shKids * 100}%`,
                      background: "#C8D2D4",
                    }}
                  ></div>
                </div>
                ...
                <div className="flex flex-wrap gap-3 mt-2 text-xs font-['IBM_Plex_Mono',monospace] text-[#404040]">
                  <span className="flex items-center gap-1">
                    <i
                      className="w-2.5 h-2.5 rounded-xs"
                      style={{ background: "#CFF1FF" }}
                    ></i>
                    PGSI 1–2 <b>{calcResults.shPct[0]}%</b>
                  </span>
                  <span className="flex items-center gap-1">
                    <i
                      className="w-2.5 h-2.5 rounded-xs"
                      style={{ background: "#2989B0" }}
                    ></i>
                    PGSI 3–7 <b>{calcResults.shPct[1]}%</b>
                  </span>
                  <span className="flex items-center gap-1">
                    <i
                      className="w-2.5 h-2.5 rounded-xs"
                      style={{ background: "#0A4561" }}
                    ></i>
                    PGSI 8+ <b>{calcResults.shPct[2]}%</b>
                  </span>
                  <span className="flex items-center gap-1">
                    <i
                      className="w-2.5 h-2.5 rounded-xs"
                      style={{ background: "#7E9296" }}
                    ></i>
                    Family &amp; friends <b>{calcResults.shPct[3]}%</b>
                  </span>
                  <span className="flex items-center gap-1">
                    <i
                      className="w-2.5 h-2.5 rounded-xs"
                      style={{ background: "#C8D2D4" }}
                    ></i>
                    Children <b>{calcResults.shPct[4]}%</b>
                  </span>
                </div>
            ...
            <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
              <h2 className="font-bold text-sm tracking-wider uppercase mb-2 text-[#1A1A1A]">
              ...
              <div className="py-2 border-b border-[#E5E5E5]">
                ...
                <div className="text-[13px] text-[#404040]">
                  children estimated to live with an adult who may benefit from
                  gambling treatment or support (OHID headcount rate, scaled)
                </div>
              </div>
              ...
              <div className="py-2">
                ...
                <div className="text-[13px] text-[#404040]">
                  healthy years lost by those children each year on the HNA
                  proxy method
                </div>
              </div>
          ...
          <aside className="space-y-5 lg:sticky lg:top-28 self-start">
            ...
            <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
              <h2 className="font-bold text-sm tracking-wider uppercase mb-1 text-[#1A1A1A]">
                {isAdvanced && <span>2 · </span>}Which survey do you want to
                count on?
              </h2>
              <p className="text-[13px] text-[#404040] mb-3 leading-relaxed">
                Three UK surveys measure gambling harm and disagree by about
                five times at the severe end.
              </p>
              ...
              <div className="space-y-2">
                ...
                {Object.entries(SURVEYS).map(([key, s]) => {
                  ...
                  return (
                    <label
                      key={key}
                      className={`grid grid-cols-[16px_minmax(0,1fr)_48px_48px_minmax(0,105px)] gap-2 items-center p-3  text-xs border cursor-pointer transition-all ${
                        isChecked
                    ...
                    >
                      ...
                      <span className="font-medium text-[#1A1A1A]">
                        ...
                        {key === "gsgb" && (
                          <span
                            title="GHUK recommended default"
                            className="text-[#166F94]"
                          >
                            ★
                          </span>
                        )}
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] text-right text-[#1A1A1A] font-semibold">
                        {s.p1plus}
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] text-right text-[#1A1A1A] font-semibold">
                        {s.p8plus}
                      </span>
                      <span className="text-[11px] text-[#404040] text-right font-normal leading-tight">
                        {s.method}
                      </span>
                    </label>
                  );
              ...
              <details className="mt-3 text-xs text-[#404040]">
                ...
                <div className="mt-2 space-y-2 text-xs text-[#404040] leading-relaxed">
                  <p>
                    It is how the question was asked, not a change in the
                    population. People disclose more gambling to a screen than
                    to an interviewer, a mode effect of roughly four percentage
                    points. The Gambling Commission cautions that push-to-web
                    GSGB must not be compared with older telephone or
                    face-to-face surveys. The independent Sturgis review (2024)
                    found a non-negligible risk that GSGB <em>overstates</em>{" "}
                    harm through topic-interest non-response, while noting that
                    older interviewer-based surveys probably <em>under</em>
                    -counted it.
                  </p>
                  <p>
                    So the honest reading is that the true figure sits somewhere
                    inside this envelope, not that one survey is right.{" "}
                    <b className="text-[#1A1A1A]">
                      GSGB if you want the official statistic
                    </b>{" "}
                    and consistency with the GHUK Health Needs Assessment.{" "}
                    <b className="text-[#1A1A1A]">APMS if you need a floor</b>{" "}
                    no one can call inflated.{" "}
                    <b className="text-[#1A1A1A]">
                      BGPS if you are answering someone quoting the old
                      face-to-face series
                    </b>{" "}
                    back at you.
                  </p>
                  <p>
                    Burden is counted from{" "}
                    <b className="text-[#1A1A1A]">PGSI 1+</b> on every basis:
                    harm does not begin at a clinical threshold. Health Survey
                    for England is not offered, because HNA v9.9 uses it as a
                    wellbeing signal rather than a prevalence basis.
                  </p>
                </div>
              </details>
            </div>
            ...
            {isAdvanced && (
              <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
                <h2 className="font-bold text-sm tracking-wider uppercase mb-1 text-[#1A1A1A]">
                  3 · Prevalence detail &amp; local variation
                </h2>
                <div className="flex border-[1.5px] border-[#1A1A1A]  overflow-hidden my-3">
                  {[
                    { k: "blend", l: "Blended ★" },
                    { k: "ga", l: "Survey (MRP)" },
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="font-semibold text-xs">
                        PGSI 1–2 (Low severity)
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] text-xs font-semibold text-[#166F94]">
                        {p12.toFixed(1)}%
                      </span>
                    </div>
                    <input
                      ...
                      value={p12}
                      onChange={(e) => {
                        setP12(parseFloat(e.target.value));
                        setSurveyMode("custom");
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={p37}
                      onChange={(e) => {
                        setP37(parseFloat(e.target.value));
                        setSurveyMode("custom");
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={p8}
                      onChange={(e) => {
                        setP8(parseFloat(e.target.value));
                        setSurveyMode("custom");
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
                  ...
                  <div>
                    <div className="flex justify-between items-baseline mb-1">
                      <span className="font-semibold text-xs">
                        Adults affected by someone else
                      </span>
                      <span className="font-['IBM_Plex_Mono',monospace] text-xs font-semibold text-[#166F94]">
                        {ao.toFixed(1)}%
                      </span>
                    ...
                    <input
                      ...
                      value={ao}
                      onChange={(e) => {
                        setAo(parseFloat(e.target.value));
                        setSurveyMode("custom");
                      }}
                      className="w-full accent-[#166F94] cursor-pointer"
                    />
            ...
            {isAdvanced && (
              <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
                <h2 className="font-bold text-sm tracking-wider uppercase mb-1 text-[#1A1A1A]">
                  4 · Health disutility (Moayeri 2020)
                </h2>
                <div className="grid grid-cols-3 gap-2 mt-3 text-center">
                  <div className="p-2 border border-[#E5E5E5] ">
                    ...
                    <input
                      ...
                      value={dw12}
                      onChange={(e) => {
                        setDw12(parseFloat(e.target.value) || 0);
                        markCustomScen();
                      }}
                  ...
                  <div className="p-2 border border-[#E5E5E5] ">
                    ...
                    <input
                      ...
                      value={dw37}
                      onChange={(e) => {
                        setDw37(parseFloat(e.target.value) || 0);
                        markCustomScen();
                      }}
                      className="w-full text-center font-['IBM_Plex_Mono',monospace] text-sm font-semibold p-1 border rounded"
                    />
                  ...
                  <div className="p-2 border border-[#E5E5E5] ">
                    ...
                    <input
                      ...
                      value={dw8}
                      onChange={(e) => {
                        setDw8(parseFloat(e.target.value) || 0);
                        markCustomScen();
                      }}
                      className="w-full text-center font-['IBM_Plex_Mono',monospace] text-sm font-semibold p-1 border rounded"
                    />
                ...
                <div className="mt-4">
                  ...
                  <input
                    ...
                    value={af}
                    onChange={(e) => {
                      setAf(parseFloat(e.target.value));
                      markCustomScen();
                    }}
                    className="w-full accent-[#166F94] cursor-pointer"
                  />
            ...
            {isAdvanced && (
              <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
                ...
                <div className="space-y-3 mt-4">
                  <div className="text-xs font-semibold text-[#1A1A1A]">
                    One-way sensitivity of total wellbeing-years
                  </div>
                  {tornadoItems.map((it, idx) => {
                    const l = Math.min(it.lo, it.hi),
                      r = Math.max(it.lo, it.hi);
                    const left = 50 + ((l - calcResults.yld) / maxDev) * 48;
                    const width = ((r - l) / maxDev) * 48;
                    return (
                      <div key={idx} className="text-xs space-y-1">
                        <div className="flex justify-between text-[11px]">
                          <span className="text-[#404040] truncate max-w-[240px]">
                            {it.l}
                          </span>
                          <b className="font-['IBM_Plex_Mono',monospace] text-[#0A4561]">
                            ±{round3sf(it.swing / 2).toLocaleString("en-GB")}
                          </b>
                        </div>
                        ...
                        <div className="text-[10px] font-['IBM_Plex_Mono',monospace] text-[#404040]">
                          {round3sf(l).toLocaleString("en-GB")} –{" "}
                          {round3sf(r).toLocaleString("en-GB")} yrs
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
              ...
              <p className="text-xs text-[#404040] leading-relaxed">
                Every estimate on this page is built from named official
                statistics and peer-reviewed evidence: GSGB 2024 (Gambling
                Commission) for prevalence and gradients; GambleAware–YouGov MRP
                local profiles (OCSI, 2024); ONS populations, Census 2021, IMD
                2019 and the Gambling Commission premises register for local
                structure; Moayeri (2020) for the health-utility decrements the
                burden figures are built on; BGPS 2010 and APMS 2023/24 as
                alternative prevalence bases; OHID (2023) and Browne, Tulloch et
                al. (2025) for costs. The full model specification, parameter
                audit table with robustness ratings, data provenance,
                limitations and citation guidance are on the{" "}
                <a
                  href="/methodology"
                  className="text-[#166F94] font-semibold hover:underline"
                >
              ...
              <p className="text-xs text-[#404040] mt-3 leading-relaxed">
                Estimates are <b>ecological</b>: they describe expected burden
                in a population given its structure and survey signal. They do
                not identify individuals, and area-level associations must not
                be read as individual-level risk.
              </p>
              <p className="text-[11.5px] text-[#404040] mt-3 pt-3 border-t border-[#E5E5E5] font-['IBM_Plex_Mono',monospace] leading-relaxed">
                Model v2.6 (presentation revision, 12 July 2026; all default
                parameters frozen as v2.5, 11 July 2026). Data vintages: GSGB
                2024 · ONS mid-2023 populations · Census 2021 ethnicity · IMD
                2019 · GambleAware/OCSI MRP profiles 2024 · Gambling Commission
                premises register July 2026 · ONS boundaries Dec 2023 · BRES
                jobs.
              </p>
            </div>
            ...
            <div className="bg-white border border-[#E5E5E5] rounded-[10px] p-5 shadow-xs">
              <h2 className="font-bold text-sm tracking-wider uppercase mb-2 text-[#1A1A1A]">
                Act locally: precedents
              </h2>
              <p className="text-xs text-[#404040] mb-3 leading-relaxed">
                Councils that have already put gambling harm into strategic and
                licensing documents, precedents for your own JSNA chapter,
                statement of licensing principles or needs assessment:
              </p>
              <div className="text-xs text-[#404040] space-y-2 leading-relaxed">
                <p>
                  <b className="text-[#1A1A1A]">Westminster</b>, Gambling
                  Vulnerability Zones and an interactive local area profile in
                  its{" "}
                  <a
                    href="https://www.westminster.gov.uk/licensing/licensing-policy-and-strategy/statement-principles-gambling-westminster"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                ...
                <p>
                  <b className="text-[#1A1A1A]">Newham</b>,{" "}
                  <a
                    href="https://www.newham.gov.uk/downloads/file/5161/current-gambling-a4-v2-online-2022-policy"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                  ...
                  plus a Local Plan cumulative-impact approach limiting
                  betting-shop clustering (400m rule) since 2016.
                </p>
                <p>
                  <b className="text-[#1A1A1A]">Sheffield</b>, gambling harms
                  chapter in the JSNA, alongside alcohol and drugs.
                </p>
                <p>
                  <b className="text-[#1A1A1A]">Brent</b>, dedicated{" "}
                  <a
                    href="https://data.brent.gov.uk/download/20d1j/698/JSNA_Gambling_AV.pdf"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                ...
                <p>
                  <b className="text-[#1A1A1A]">Brighton &amp; Hove</b>,{" "}
                  <a
                    href="https://www.brighton-hove.gov.uk/joint-strategic-needs-assessment-jsna/area-profiles/local-area-profile-assessing-gambling-related-harm-2024"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                    local area profile for assessing gambling-related harm
                    (2024)
                  </a>{" "}
                  within the JSNA.
                </p>
                <p>
                  <b className="text-[#1A1A1A]">Leeds</b>,{" "}
                  <a
                    href="https://www.leeds.gov.uk/licensing/licensing-policies/statement-of-licensing-policy"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                    statement of licensing policy (2025)
                  </a>{" "}
                  with a cumulative impact assessment approved January 2026.
                </p>
                <p>
                  <b className="text-[#1A1A1A]">Swindon</b>, rapid health needs
                  assessment on the scale and costs of gambling harms.
                </p>
                <p>
                  <b className="text-[#1A1A1A]">
                  ...
                  ,{" "}
                  <a
                    href="https://www.gamblingcommission.gov.uk/news/article/spotlight-on-innovative-local-public-health-approaches-to-reducing-gambling"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                  ...
                  and a whole-system harm-reduction programme.
                </p>
                <p>
                  <b className="text-[#1A1A1A]">Hertfordshire</b>,{" "}
                  <a
                    href="https://www.hertshealthevidence.org/microsites/jsna/jsna-documents/gambling-related-harms-jsna-briefing-2022.pdf"
                    target="_blank"
                    rel="noreferrer"
                    className="text-[#166F94] underline hover:text-[#0A4561]"
                  >
                    JSNA briefing on gambling-related harms (2022)
                  </a>{" "}
                  and a county gambling harms strategy.
                </p>
              </div>
              <p className="text-xs text-[#404040] italic mt-3 pt-2.5 border-t border-[#E5E5E5] leading-relaxed">
                See also the LGA’s{" "}
                <a
                  href="https://www.local.gov.uk/publications/tackling-gambling-related-harm-whole-council-approach"
                  target="_blank"
                  rel="noreferrer"
                  className="text-[#166F94] underline hover:text-[#0A4561]"
                >
                ...
                guidance and the Gambling Commission’s{" "}
                <a
                  href="https://www.gamblingcommission.gov.uk/authorities/guide/reducing-gambling-harms-resources"
                  target="_blank"
                  rel="noreferrer"
                  className="text-[#166F94] underline hover:text-[#0A4561]"
                >
            ...
            <div className="border-l-4 border-[#0A4561] bg-[#FBF4E7] p-4  text-xs text-[#1A1A1A] leading-relaxed shadow-xs">
              ...
              produced by products, environments and industry practices, not by
              a category of person. PGSI labels are retained only to reference
              source statistics.
            </div>
          </aside>
        </main>
      ...
      <div id="print-report" className="hidden" aria-hidden="true">
        ...
        <h1>
          Gambling harm burden estimate,{" "}
          {selectedLA ? selectedLA.n : "Custom population"}
        </h1>
        <div className="pr-meta">
          ...
          {activeScen === "custom"
            ? "custom"
            : activeScen === "moa"
              ? "Adjusted weights (Moayeri 2020 / GHS-AO)"
              : activeScen === "ao30"
                ? "Family-side 30% transfer floor"
                : "Central (published defaults)"}{" "}
          · prevalence:{" "}
          {surveyMode === "custom" ? "custom bands" : SURVEYS[surveyMode].name}{" "}
          · basis:{" "}
          {basisMode === "ga"
            ? "survey (MRP)"
            : basisMode === "blend"
        ...
        <div className="pr-row">
          <span>
            Years of health and wellbeing lost annually (HNA utility-decrement
            basis), incl. family, friends and children
          </span>
        ...
        <div className="pr-row">
          ...
          <b>
            {fmtMoney(calcResults.ohidLo)}–{fmtMoney(calcResults.ohidHi)}/yr
          </b>
        </div>
        <div className="pr-row">
          <span>
            Full social-cost accounting (Browne, Tulloch et al. 2025 transfer;
            non-additive)
          </span>
        ...
        <div className="pr-row">
          <span>
            Sensitivity: societal wellbeing valuation (£
            {(calcResults.qalyK * 1000).toLocaleString("en-GB")}/healthy yr;
            upper-envelope framing, not a cashflow cost)
          </span>
        ...
        <div className="pr-row">
          ...
          <b>
            {round3sf(calcResults.n8).toLocaleString("en-GB")} (
            {calcResults.p8e.toFixed(1)}%)
          </b>
        </div>
        <div className="pr-row">
          <span>, PGSI 3–7</span>
          <b>
            {round3sf(calcResults.n37).toLocaleString("en-GB")} (
            {calcResults.p37e.toFixed(1)}%)
          </b>
        </div>
        <div className="pr-row">
          <span>, PGSI 1–2</span>
          <b>
            {round3sf(calcResults.n12).toLocaleString("en-GB")} (
            {calcResults.p12e.toFixed(1)}%)
          </b>
        </div>
        <div className="pr-row">
          <span>
            Adults affected by someone else's gambling (children additional,
            uncounted)
          </span>
        ...
        <div className="pr-row">
          <span>
            Adults at PGSI 8+ likely not receiving structured treatment (≈95%
            gap)
          </span>
        ...
        {selectedLA && (
          <>
            <div className="pr-sec">
              LOCAL AREA PROFILE EVIDENCE, LICENSED PREMISES (GAMBLING
              COMMISSION REGISTER, JULY 2026)
            </div>
            {PREM_LABELS.map((l, i) => (
        ...
        <div className="pr-note">
          Burden = population × prevalence × health disutility, counted from
          PGSI 1+ and including family and friends. Prevalence: GSGB 2024 (
          {calcResults.p12}/{calcResults.p37}/{calcResults.p8}% by category),
          locally adjusted ×{calcResults.adj8.toFixed(2)} at PGSI 8+ under the{" "}
          {basisMode === "ga"
            ? "survey (MRP)"
            : basisMode === "blend"
              ? "blended"
          ...
          basis
          {calcResults.ceilHit
            ? "; 25% PGSI 1+ ceiling binding (categories rescaled)"
            : ""}
          . Disability weights: Moayeri (2020) basis ({calcResults.dw12}/
          {calcResults.dw37}/{calcResults.dw8}; family and friends{" "}
          {calcResults.dwao12}/{calcResults.dwao37}/{calcResults.dwao8} by
          source severity; directly elicited disability weights are not used in
          this tool). Causal-attribution factor {calcResults.af.toFixed(2)}{" "}
          applied to all weights. Prevalence basis:{" "}
          {surveyMode === "custom"
            ? "custom hand-set bands"
            : SURVEYS[surveyMode].name}
          . Any sampling range shown reflects GSGB intervals only; full
          uncertainty spans the scenario envelope. “Years of health and
          wellbeing lost” uses the Moayeri (2020) utility-decrement basis and is
          not comparable to GBD YLD. Deprivation is modelled for English
          authorities (GSGB D.8 × IMD 2019); online gambling has no local supply
          term; estimates are ecological.
        </div>

        <div className="pr-foot">
          Gambling Harm UK Burden Calculator v2.6 (defaults frozen as v2.5, 11
          July 2026); registered charity 1196530. Estimates are ecological
          population-level planning figures, not individual-level risk or
          surveillance counts. Children are counted via the OHID proxy method
          (see methods); online gambling has no local supply term. Full
          methodology, parameter audit and citation guidance: the calculator's
          methodology & audit page.
        </div>
      </div>
    </div>
